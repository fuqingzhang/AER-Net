...
#         ])
#
#     def _sync_transform(self, img, mask, edge_gt):
#         if self.mode == 'train' and self.data_aug:
#             if random.random() < 0.5:
#                 img = img.transpose(Image.FLIP_LEFT_RIGHT)
#                 mask = mask.transpose(Image.FLIP_LEFT_RIGHT)
#                 edge_gt = edge_gt.transpose(Image.FLIP_LEFT_RIGHT)
...
#             edge_gt = edge_gt.resize((ow, oh), Image.NEAREST)
#             if short_size < crop_size:
#                 padh = crop_size - oh if oh < crop_size else 0
#                 padw = crop_size - ow if ow < crop_size else 0
#                 img = ImageOps.expand(img, border=(0, 0, padw, padh), fill=0)
#                 mask = ImageOps.expand(mask, border=(0, 0, padw, padh), fill=0)
#                 edge_gt = ImageOps.expand(edge_gt, border=(0, 0, padw, padh), fill=0)
#             w, h = img.size
#             x1 = random.randint(0, w - crop_size)
#             y1 = random.randint(0, h - crop_size)
#             img = img.crop((x1, y1, x1 + crop_size, y1 + crop_size))
#             mask = mask.crop((x1, y1, x1 + crop_size, y1 + crop_size))
#             edge_gt = edge_gt.crop((x1, y1, x1 + crop_size, y1 + crop_size))
#             if random.random() < 0.5:
#                 img = img.filter(ImageFilter.GaussianBlur(radius=random.random()))
#             img, mask, edge_gt = np.array(img), np.array(mask), np.array(edge_gt)
#             img = self.transform(img)
#             mask = transforms.ToTensor()(mask)
#             edge_gt = transforms.ToTensor()(edge_gt)
#         else:
#             img = img.resize((self.base_size, self.base_size), Image.BILINEAR)
#             mask = mask.resize((self.base_size, self.base_size), Image.NEAREST)
#             edge_gt = edge_gt.resize((self.base_size, self.base_size), Image.NEAREST)
#             img, mask, edge_gt = np.array(img), np.array(mask), np.array(edge_gt)
#             img = self.transform(img)
#             mask = transforms.ToTensor()(mask)
#             edge_gt = transforms.ToTensor()(edge_gt)
#         return img, mask, edge_gt
#     def __getitem__(self, item):
#         img_name = self.img_names[item]
#         img_path = osp.join(self.data_dir, 'images', img_name)
#         label_path = osp.join(self.data_dir, 'masks', img_name)
#         label_path = osp.join(self.data_dir, 'canny_edge', img_name)
#         img = Image.open(img_path).convert('RGB')
#         mask = Image.open(label_path)
#         edge_gt = Image.open(label_path)
#         img, mask, edge_gt = self._sync_transform(img, mask, edge_gt)
#         return img, mask, edge_gt
#
#     def __len__(self):
